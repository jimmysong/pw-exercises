...
>>> # get the signing raw paths
>>> raw_paths = psbt_obj_1.get_signing_raw_paths()  #/
>>> # get the private keys from those raw paths
>>> private_keys = hd_priv.get_private_keys(raw_paths)  #/
>>> # sign the psbt
>>> print(psbt_obj_1.sign(private_keys))  #/
True
...
class SessionTest(TestCase):
    ...
    def test_apply(self):
        HDPrivateKey.get_private_keys = get_private_keys
        PSBT.combine = psbt_combine
        PSBT.create = create
        PSBT.finalize = psbt_finalize
        PSBT.final_tx = final_tx
        PSBT.get_signing_raw_paths = get_signing_raw_paths
        PSBT.sign = sign
