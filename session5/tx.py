...
class Tx:
    ...
    def clone(self):
        return self.__class__.parse(BytesIO(self.serialize()))
    ...
    def sig_hash_bip143(self, input_index, redeem_script=None, witness_script=None):
        ...
        if witness_script:
            # p2wsh or p2sh-p2wsh ScriptCode is the serialized WitnessScript
            ...
            # turn the hash160 into the serialized p2pkh script, this is the ScriptCode
    ...
    def verify_input(self, input_index):
        ...
        if script_pubkey.is_p2sh():
            ...
            if redeem_script.is_p2wpkh():
                ...
                witness_script = WitnessScript.convert(tx_in.witness.items[-1])
                # calculate the z using sig_hash_bip143
...
class TxIn:
    ...
    def __repr__(self):
        return '{}:{}:{}:{}'.format(
            ...
            int_to_little_endian(self.sequence, 4).hex(),
        )
    ...
    def finalize_p2wpkh(self, sig, sec, redeem_script=None):
        '''Puts together the Witness for a p2wpkh or p2sh-p2wpkh input so the input verifies.'''
        # the ScriptSig for p2pkh is empty or the redeem_script
        if redeem_script:
