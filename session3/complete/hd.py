...
class HDPrivateKey:
    ...
    def _prv(self, version):
        ...
        raw = version
        # add depth, which is 1 byte
        raw += int_to_byte(self.depth)
        # add the fingerprint
        raw += self.fingerprint
        # add the child number 4 bytes big endian using int_to_big_endian
        raw += int_to_big_endian(self.child_number, 4)
        # add the chain code
        raw += self.chain_code
        # add the 0 byte and the private key's secret in big endian
        raw += b'\x00' + int_to_big_endian(self.private_key.secret, 32)
    ...
    def from_seed(cls, seed, path, testnet=False):
        ...
        private_key = PrivateKey(secret=big_endian_to_int(raw[:32]))
        chain_code = raw[32:]
        root = cls(
            private_key=private_key,
            chain_code=chain_code,
    ...
    def child(self, index, hardened=False):
        ...
        if hardened:
            index += 0x80000000
            pk = int_to_big_endian(self.private_key.secret, 32)
            data = b'\x00' + pk + int_to_big_endian(index, 4)
            raw = HMAC(
                key=self.chain_code, msg=data, digestmod=sha512).digest()
        else:
            data = sec + int_to_big_endian(index, 4)
            raw = HMAC(
                key=self.chain_code, msg=data, digestmod=sha512).digest()
        secret = (big_endian_to_int(raw[:32]) +
...
class HDPublicKey:
    ...
    def _pub(self, version):
        ...
        raw = version
        # add the depth in one byte
        raw += int_to_byte(self.depth)
        # add the fingerprint
        raw += self.fingerprint
        # add the child number in 4 bytes big-endian
        raw += int_to_big_endian(self.child_number, 4)
        # add the chain code
        raw += self.chain_code
        # add the SEC pubkey
    ...
    def child(self, index):
        ...
        point = self.point + big_endian_to_int(raw[:32]) * G
        chain_code = raw[32:]
        depth = self.depth + 1
        fingerprint = hash160(sec)[:4]
        child_number = index
...
class HDTest(TestCase):
    ...
    def test_zprv(self):
        ...
        self.assertTrue(derived_pub.p2wpkh_script_pubkey().is_p2wpkh())
